...
public class TrDisplayForCashier extends TrDisplay {
	/**
	 * 
	 */
	...
	private static TrDisplayForCashier instance;
	...
	public TrDisplayForCashier(Dimension dim){
		super(dim);
		initPriceDisplay();
		setMinimumSize(dim);
		setMaximumSize(dim);
	...
	//##########  DISPLAY
		...
		public void  showArticleOnDisplayForSale(float quantity, Article article, Cart cart){
			String articleAmountText = quantity+" "+article.getUnit();
			String articleNameText = article.getName();
			String articlePriceText = article.getPriceGross()+" € / "+article.getUnit();
			String articleSum = String.format("%.2f", (quantity*article.getPriceGross())).toString()+" €";
		...
		public void  showArticleOnDisplayForCancellation(float quantity, Article article, Cart cart){
			String articleAmountText = quantity+" "+article.getUnit();
			String articleNameText = article.getName();
			String articlePriceText = - article.getPriceGross()+" € / "+article.getUnit();
			String articleSum = String.format("%.2f", (quantity*article.getPriceGross())).toString()+" €";
		...
		public final  ImageIcon getLogo(){
		...
		public static synchronized TrDisplayForCashier getInstance () {
			if (TrDisplayForCashier.instance == null) {
				TrDisplayForCashier.instance = new TrDisplayForCashier(new Dimension(TrDimensions.mainAreaWidth-TrDimensions.displayPanelBorder*2, TrDimensions.displayHeight));
			}
			return TrDisplayForCashier.instance;
		}
